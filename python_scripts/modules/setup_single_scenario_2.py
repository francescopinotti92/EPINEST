...
def setup_single_scenario_Bangladesh_2( dx, scenarioID, path_folder_config, path_folder_output ):
    r"""
    ...
    #==== 0) Obtain fluxes-related parameters and included areas
    

    bts = dx['general_settings']['bts']
    n_areas = dx['popgenerator_settings']['flux_params']['n_areas']
    districts = dx['popgenerator_settings']['flux_params']['districts'] # select Dhaka or Chattogram
    ...
    # select generative algorithm
    if 'mode' in dx['popgenerator_settings']['flux_params']:
        mode_flux = dx['popgenerator_settings']['flux_params']['mode'] # read value from configuration file
    else:
        mode_flux = 'gen_dirichlet' # default value
    
    if ( mode_flux == 'gen_dirichlet' ):
        # fits a dirichlet distribution to each area
        _ , areas_included = pg.compute_area_market_fluxes_parameters_dirichlet_bangladesh( paths_read_raw_flux, 
                                                            bts, 
                                                            n_areas, 
                                                            districts, 
        ...
        # uses raw fluxes
        n_markets         = dx['popgenerator_settings']['market_params']['n_markets'] # get number of markets
        _ , areas_included = pg.compute_area_market_fluxes_parameters_raw_bangladesh( paths_read_raw_flux, 
                                                            bts, 
                                                            n_areas,
                                                            n_markets, 
                                                            districts, 
        ...
        raise ValueError( "flux generative mode not recognized." )
    ...
    for i in range( n_realizations ):
        ...
        if ( mode_flux == 'gen_dirichlet' ):
            ...
            pg.compute_area_market_flux_raw( gdf, bts, path_flux_params, path_area_market_flux )           
