...
// basic code to run a simple simulation with a single strain
int main( int argc, char* argv[] ) {
    ...
    //ExampleModel* epiModel = MakeExampleModel( model_settings["epi_model_path"].get<std::string>() );
    std::string epi_params_path = model_settings["epi_model_path"].get<std::string>() ;
    SingleStrainSEIRMaker epiModelMaker( epi_params_path ) ;
    SEIR* epiModel = epiModelMaker.makeModel() ;
    ...
    for ( uint sim = 0; sim < Nsim; ++sim ) {
        ...
        for ( uint day = 0; day < nDays; day++ ) {
            ...
            //std::cout << day << " " << mySimulator->getTotalNumberBirds() << std::endl ;

            // allow external infections starting from Tintro
            
            if ( day >= Tintro ) mySimulator->sparkInfectionFarms( 0, sparkRate, randGen );
             
            // loop over hours in a single day
